...
// Get random questions for parts 1, 2, and 3
...
// Get selected questions based on genres for parts 1, 2, and 3
...
router.post('/prompts', async (req, res) => {
  const { part_id, question, time_limit, genre } = req.body;
  try {
    await pool.query('INSERT INTO speaking_prompts (part_id, question, time_limit, genre) VALUES ($1, $2, $3, $4)', [part_id, question, time_limit, genre]);
    res.status(201).json({ message: 'Speaking prompt added' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
...
// Update a speaking prompt
router.put('/prompts/:id', async (req, res) => {
  const { id } = req.params;
  const { part_id, question, time_limit, genre } = req.body;
  try {
    await pool.query('UPDATE speaking_prompts SET part_id = $1, question = $2, time_limit = $3, genre = $4 WHERE id = $5', [part_id, question, time_limit, genre, id]);
    res.json({ message: 'Speaking prompt updated' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
...
// Delete a speaking prompt
router.delete('/prompts/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query('DELETE FROM speaking_prompts WHERE id = $1', [id]);
    res.json({ message: 'Speaking prompt deleted' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
