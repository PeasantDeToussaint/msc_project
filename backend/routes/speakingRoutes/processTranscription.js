...
const promptFilePath = path.resolve(__dirname, '../../utils', 'instruction_speech.txt');
const promptContent = fs.readFileSync(promptFilePath, 'utf-8');

// Define the POST route for processing the transcribed speech
router.post('/processTranscription', async (req, res) => {
...
function parseFeedback(text) {
  const feedback = {
    ...
    coherenceAndCohesion: { score: 'N/A', text: 'No feedback available' },
    lexicalResource: { score: 'N/A', text: 'No feedback available' },
    grammaticalRangeAndAccuracy: { score: 'N/A', text: 'No feedback available' },
  };
  ...
  sections.forEach(section => {
    if (section.startsWith('Overall-Score:')) {
      ...
      feedback.coherenceAndCohesion.score = extractScore(section);
      feedback.coherenceAndCohesion.text = extractFeedbackText(section, 'Coherence-and-Cohesion-Feedback:');
    } else if (section.startsWith('Lexical-Resource-Score:')) {
      feedback.lexicalResource.score = extractScore(section);
      feedback.lexicalResource.text = extractFeedbackText(section, 'Lexical-Resource-Feedback:');
    } else if (section.startsWith('Grammatical-Range-and-Accuracy-Score:')) {
      feedback.grammaticalRangeAndAccuracy.score = extractScore(section);
      feedback.grammaticalRangeAndAccuracy.text = extractFeedbackText(section, 'Grammatical-Range-and-Accuracy-Feedback:');
    }
  });
