...
router.get('/getData', authorize, async (req, res) => {
  try {
    const { id } = req.user; // Extract user ID from the JWT token
    const { rows: essays } = await pool.query('SELECT essay, prompt, overall_score, created_at FROM essays WHERE user_id = $1', [id]);
    res.json({ essays });
  } catch (err) {
    console.error('Error fetching essays:', err.message);
    res.status(500).json({ msg: 'Server error' });
